    ...
    <div class="w-full flex grow flex-col overflow-auto h-0 bg-gray-200 justify-center items-center p-2">
      ...
      <div class="bg-white p-2">
        <form action="#" class="w-full flex flex-col sm:flex-row flex-wrap text-xs">
          <div class="w-full p-1 sm:w-6/12 md:w-6/12 lg:w-4/12 flex flex-col">
          ...
          <div class="w-full p-1 sm:w-12/12 md:w-12/12 lg:w-4/12 flex items-end justify-end">
...
const printPreview = async()=>{
  ...
  inject_params();
  useCommonStore().loading_full = true;
  const { data, error, status } = await useMyFetch("/report/distance/pdf_preview", {
    method: 'get',
    headers: {
      'Authorization': `Bearer ${token.value}`,
      'Accept': 'application/json'
...
const downloadExcel = async()=>{  
  inject_params();
  useCommonStore().loading_full = true;
  const { data, error, status } = await useMyFetch("/report/distance/excel_download", {
    method: 'get',
    headers: {
      'Authorization': `Bearer ${token.value}`,
      'Accept': 'application/json'

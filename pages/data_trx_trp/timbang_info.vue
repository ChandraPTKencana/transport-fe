  ...
  <div class="w-full h-full flex flex-col">
    ...
    <div class="w-full flex grow flex-col overflow-auto h-0">
      ...
      <TableView :thead="fields_thead" :selected="selected" @setSelected="selected = $event" :tbody="trx_trps" :fnCallData="callData" :scrolling="scrolling" @setScrollingPage="scrolling.page=$event" @doFilter="searching()"  @setCheckbox="checkbox_arr=$event" :deep_state="deep_state">
        <template #[`timbang_val1`]="{item}">
          <IconsLine v-if="!item.timbang_val1"/>
          <IconsCheck v-else/>
        </template>
    ...
    <LazyFormsTrxTrpTimbangInfo :show="forms_trx_trp_show" :fnClose="()=>{forms_trx_trp_show=false}" :id="forms_trx_trp_id" :p_data="trx_trps" />
    <LazyFormsTrxTrpTimbangInfoValidasi :show="forms_trx_trp_valid_show" :fnClose="()=>{forms_trx_trp_valid_show=false}" :id="forms_trx_trp_valid_id" :p_data="trx_trps" :is_view="forms_trx_trp_is_view" @setID="forms_trx_trp_valid_id=$event" @setIndex="selected=$event"/>
  </div>
</template>
...
definePageMeta({
  ...
  middleware: [
    function (to, from) {
      if (!useAuthStore().checkPermission('trp_trx.timbang_info.views')){
...
const checkStatus=(data)=>{
  ...
  if(data.timbang_val1 == 1) return "!bg-blue-300"; 
...
const { data: dt_async } = await useAsyncData(async () => {
  ...
  const [data1, data2] = await Promise.all([
    useMyFetch("/trx_trp/timbang_infos", {
      method: 'get',
      headers: {
        'Authorization': `Bearer ${token.value}`,
        'Accept': 'application/json'
      },
      params:{filter_status:'Undone'},
      retry: 0,
    }),
...
const callData = async () => {
  ...
  const { data, error, status } = await useMyFetch("/trx_trp/timbang_infos", {
    method: 'get',
    headers: {
      'Authorization': `Bearer ${token.value}`,
      'Accept': 'application/json'
...
const multiUnVal = async() => {
  ...
  const { data, error, status } = await useMyFetch("/trx_trp/timbang_info/clear_valval1", {
    method: "post",
    headers: {
      'Authorization': `Bearer ${token.value}`,
      'Accept': 'application/json',
...
const fields_thead=ref([
  ...
  {key:"timbang_val1",label:"Logistik",filter_on:1,type:"select",select_item:[{k:'1',v:'Approve'},{k:'0',v:'Unapprove'}]},
  ...
  {key:"jenis",label:"Jenis",filter_on:1,type:"select",select_item:['TBS','TBSK','CPO','PK','LAIN','TUNGGU','CANGKANG']},
...
const enabled_view = computed(()=>{ 
  ...
  && useUtils().checkPermission('trp_trx.timbang_info.view');
  return result;
})
...
const enabled_edit = computed(()=>{  
  ...
  && useUtils().checkPermission('trp_trx.timbang_info.modify');
  return result;
})
...
const enabled_validasi = computed(()=>{  
  ...
  // )
  && useUtils().checkPermissions(['trp_trx.timbang_info.val1']);
  return result;
})
...
const downloadExcel = async()=>{  
  ...
  useCommonStore().loading_full = true;
  params.filter_status = filter_status.value;
  const { data, error, status } = await useMyFetch("/trx_trp/timbang_info/download_excel", {
    method: 'get',
    headers: {
      'Authorization': `Bearer ${token.value}`,
      'Accept': 'application/json'

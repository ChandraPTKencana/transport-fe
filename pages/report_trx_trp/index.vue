...
definePageMeta({
  ...
  middleware: [
    function (to, from) {
      ...
      //   return navigateTo('/');
      if (!useAuthStore().checkRole(["SuperAdmin","Logistic",'Finance', 'MIS']))
      return navigateTo('/');
...
const inject_params = () => {
  params.like = "";
  if (search.value != "") {
    params.like = `id:%${search.value}%,xto:%${search.value}%,jenis:%${search.value}%,pv_no:%${search.value}%,ticket_a_no:%${search.value}%,ticket_b_no:%${search.value}%,no_pol:%${search.value}%,supir:%${search.value}%,kernet:%${search.value}%`;
  }
  params.sort = "";
  if (sort.value.field) {

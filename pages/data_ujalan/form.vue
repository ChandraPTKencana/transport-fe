<template>
  <div class="w-full h-full flex flex-col">
    <HeaderCustom :title="'Form Ujalan'" :back="true" />
                ...
                <template v-for="(detail, index) in details" :key="index">
                  <tr v-if="detail.status!='Remove'"  :data-index="index" draggable="true" @dragstart="handleDragStart($event,index)" @dragover.prevent @drop="handleDrop($event,index)">
                    ...
                    <td class="cell">
                      <div class="w-full h-full flex items-center justify-center">                       
                        <InputPointFormat
                        :disabled="disabled || ujalan.type=='used' || ujalan.type=='transfer'"  
                    ...
                    <td class="cell">
                      <div class="w-full h-full flex items-center justify-center">
                        <InputPointFormat 
                        :disabled="disabled || ujalan.type=='in'" 
...
const { data: dt_async } = await useAsyncData(async () => {
  ...
  if (id !== undefined && id !== "") {
    ...
    const { data, error, status }: any = await useMyFetch("/api/ujalan", {
      method: 'get',
      headers: {
  ...
  return {ujalan,exclude_lists,details};
    
});

const ujalan:any = ref(dt_async.value?.ujalan || {
    id: -1,
    type: "used",
    note: "",
    // warehouse: {
...
const doSave = async () => {
  ...
  let data_in: Record<string, any> = {
    "type": ujalan.value.type,
    "note": ujalan.value.note,
    // "qty_in": ujalan.value.qty_in,
    // "qty_out": ujalan.value.qty_out,
    // "item_id": ujalan.value.item.id,
    "warehouse_id": ujalan.value.warehouse.id,
    "warehouse_target_id": ujalan.value.warehouse_target.id,

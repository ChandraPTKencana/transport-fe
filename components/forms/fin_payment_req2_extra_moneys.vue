...
const doSave = async () => {
  ...
  data_in.append("extra_money_trxs", JSON.stringify(selected_items.value));
  ...
  const { data, error, status } = await useMyFetch("/fin_payment_req2/extra_money_trx", {
    method: $method,
    headers: {
...
const callData = async () => {
  useCommonStore().loading_full = true;
  const { data, error, status } = await useMyFetch("/fin_payment_req2/get_extra_money_trx_unprocessed", {
    method: 'get',
    headers: {
      'Authorization': `Bearer ${token.value}`,
      'Accept': 'application/json'

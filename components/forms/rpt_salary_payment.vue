                  ...
                  <tbody ref="to_move">
                    <template v-for="(detail, index) in source" :key="index">
                      <tr v-if="detail.p_status!='Remove'"  :data-index="index">
                        ...
                        <td :class="[detectDiff(detail),parseFloat(detail.uj_gaji)!=parseFloat(detail.uj_gaji_manual)?'font-bold':'']">{{ pointFormat(detail.uj_gaji_manual) }}</td>
                        <td :class="detectDiff(detail)">{{ pointFormat(detail.uj_makan) }}</td>
                        <td :class="[detectDiff(detail),parseFloat(detail.uj_makan)!=parseFloat(detail.uj_makan_manual)?'font-bold':'']">{{ pointFormat(detail.uj_makan_manual) }}</td>
                        <td :class="detectDiff(detail)">{{ pointFormat(detail.uj_dinas) }}</td>
                        <td :class="[detectDiff(detail),parseFloat(detail.uj_dinas)!=parseFloat(detail.uj_dinas_manual)?'font-bold':'']">{{ pointFormat(detail.uj_dinas_manual) }}</td>
                        <td :class="detectDiff(detail)">{{ detail.uj_manual_reason }}</td>

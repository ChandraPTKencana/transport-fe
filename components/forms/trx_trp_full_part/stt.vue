...
props.standby_trxs?.forEach((x,y)=>{
    save_state.value.push("");
    save_do.value.push("");
})

const doValidate = async (st_idx,paid_state) => {
  save_state.value[st_idx] = "PROSES...";
  save_do.value[st_idx] = "VAL";
  ...
  save_state.value[st_idx] = "BERHASIL";
}

const doUnValidate = async (st_idx) => {
  save_state.value[st_idx] = "PROSES...";
  save_do.value[st_idx] = "UNVAL";
  useCommonStore().loading_full = true;
  field_errors.value = {};
